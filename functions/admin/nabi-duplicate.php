...
/**
 * nabi_Duplicate class
 */
class nabi_Duplicate {
	...
	public static function duplicate_post() {
		...
		if ( ! ( isset( $_GET['post'] ) || isset( $_POST['post'] )  || ( isset( $_REQUEST['action'] ) && 'duplicate_post' == $_REQUEST['action'] ) ) ) {
			wp_die( __( 'Aucun post à dupliquer n\'a été fourni.', 'nabi' ) );
		}
		...
		if ( isset( $post ) && $post != null ) {
			...
			wp_die( __( 'Erreur lors de la création de l\'article. L\'article original n\'a pu être trouvé.', 'nabi' ) . ' : ' . $post_id );
