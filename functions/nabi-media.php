 ...
 */
if ( ! function_exists( 'nabi_media_extra_column' ) && ! function_exists( 'nabi_media_extra_column_value' ) ) {
	function nabi_media_extra_column( $cols ) {
		// Add column
		$cols["alt"] = "ALT";
		$cols["media_url"] = "URL";
		return $cols;
	}
	function nabi_media_extra_column_value($column_name, $id) {
		// Add value
		if($column_name == 'alt') {
	...
	add_filter('manage_media_columns', 'nabi_media_extra_column');
	add_action('manage_media_custom_column', 'nabi_media_extra_column_value', 10, 2);
}
 ...
 */
function nabi_manage_media_columns( $columns) {
	// Remove Comments and Author Columns
	unset( $columns['comments'], $columns['author'] );
	return $columns;
}
add_action( 'manage_media_columns', 'nabi_manage_media_columns', 10, 2 );
 ...
 */
	...
	function nabi_media_category_update_count_callback( $terms = array(), $taxonomy = 'category' ) {
		global $wpdb;

		// default taxonomy
		$taxonomy = 'category';
		// add filter to change the default taxonomy
		$taxonomy = apply_filters( 'nabi_media_category_taxonomy', $taxonomy );
	...
	function nabi_media_category_init() {
		// Default taxonomy
		$taxonomy = 'category';
		// Add filter to change the default taxonomy
		$taxonomy = apply_filters( 'nabi_media_category_taxonomy', $taxonomy );
		...
		if ( taxonomy_exists( $taxonomy ) ) {
			...
			$args = array(
				...
				'update_count_callback' => 'nabi_media_category_update_count_callback',
				'public'				=> false
			);
			register_taxonomy( $taxonomy, array( 'attachment' ), $args );
		}
	}
	add_action( 'init', 'nabi_media_category_init' );


	/** change default update_count_callback for category taxonomy */
	function nabi_media_category_change_category_update_count_callback() {
		...
		if ( $taxonomy == 'category' ) {
			...
			$new_arg = 'nabi_media_category_update_count_callback';
		}
	}
	add_action( 'init', 'nabi_media_category_change_category_update_count_callback', 100 );
	...
	if ( is_admin() ) {
		...
		function nabi_media_category_set_attachment_category( $post_ID ) {
		...
		add_action( 'add_attachment', 'nabi_media_category_set_attachment_category' );
		add_action( 'edit_attachment', 'nabi_media_category_set_attachment_category' );


		/** Custom walker for wp_dropdown_categories, based on https://gist.github.com/stephenh1988/2902509 */
		class nabi_media_category_walker_category_filter extends Walker_CategoryDropdown{
		...
		function nabi_media_category_add_category_filter() {
			global $pagenow;
			if ( 'upload.php' == $pagenow ) {
				...
				if ( $taxonomy != 'category' ) {
					$dropdown_options = array(
						...
						'orderby'         => 'name',
						'show_count'      => true,
						'walker'          => new nabi_media_category_walker_category_filter(),
						'value'           => 'slug'
					);
				} else {
					$dropdown_options = array(
		...
		add_action( 'restrict_manage_posts', 'nabi_media_category_add_category_filter' );


		/** Add custom Bulk Action to the select menus */
		function nabi_media_category_custom_bulk_admin_footer() {
			// default taxonomy
			$taxonomy = 'category';
			// add filter to change the default taxonomy
			$taxonomy = apply_filters( 'nabi_media_category_taxonomy', $taxonomy );
		...
		add_action( 'admin_footer-upload.php', 'nabi_media_category_custom_bulk_admin_footer' );


		/** Handle the custom Bulk Action */
		function nabi_media_category_custom_bulk_action() {
			global $wpdb;
			...
			if ( substr( $sAction, 0, 16 ) != 'nabi_media_category_' ) {
				return;
			}
			...
			foreach( $post_ids as $post_id ) {
				...
				if ( is_numeric( str_replace( 'nabi_media_category_add_', '', $sAction ) ) ) {
					$nCategory = str_replace( 'nabi_media_category_add_', '', $sAction );
				...
				} else if ( is_numeric( str_replace( 'nabi_media_category_remove_', '', $sAction ) ) ) {
					$nCategory = str_replace( 'nabi_media_category_remove_', '', $sAction );
			...
			nabi_media_category_update_count_callback();

			wp_redirect( $sendback );
			exit();
		}
		add_action( 'load-upload.php', 'nabi_media_category_custom_bulk_action' );


		/** Display an admin notice on the page after changing category */
		function nabi_media_category_custom_bulk_admin_notices() {
		...
		function nabi_media_category_ajax_query_attachments_args( $query = array() ) {
		...
		add_filter( 'ajax_query_attachments_args', 'nabi_media_category_ajax_query_attachments_args' );


		/** Custom walker for wp_dropdown_categories for media grid view filter */
		class nabi_media_category_walker_category_mediagridfilter extends Walker_CategoryDropdown {
		...
		function nabi_media_category_save_attachment_compat() {
		...
		add_action( 'wp_ajax_save-attachment-compat', 'nabi_media_category_save_attachment_compat', 0 );


		/** Add category checkboxes to attachment details on insert media popup */
		function nabi_media_category_attachment_fields_to_edit( $form_fields, $post ) {
			...
			foreach ( get_attachment_taxonomies( $post->ID ) as $taxonomy ) {
				...
				if ( $t['hierarchical'] ) {
					ob_start();
						...
						wp_terms_checklist( $post->ID, array( 'taxonomy' => $taxonomy, 'checked_ontop' => false, 'walker' => new nabi_media_category_walker_media_taxonomy_checklist() ) );
		...
		add_filter( 'attachment_fields_to_edit', 'nabi_media_category_attachment_fields_to_edit', 10, 2 );


		/** Custom walker for wp_dropdown_categories for media grid view filter */
		class nabi_media_category_walker_media_taxonomy_checklist extends Walker {
